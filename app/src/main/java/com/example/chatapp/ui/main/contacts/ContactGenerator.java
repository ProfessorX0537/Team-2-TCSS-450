...
public class ContactGenerator {
    ...
    private static final String[] testNames = {"Deanna Smith","Heidi Carney","Sean Thomas","Kyle Travis","Bethany Gilbert","Linda Robles",
            ...
            "Dr. Gabriel Thompson","Alexandria Moses","Chad Greene","Jamie Morrison","Julie Werner","Cameron Pierce","Brittany Williamson"};

    static {
        CARDS = new ContactCard[testNames.length];

        for (int i = 0; i < CARDS.length; i++) {
            CARDS[i] = new ContactCard.Builder(testNames[i]).build();
